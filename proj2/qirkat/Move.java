...
class Move {
    ...
    static Move move(Move move0, Move move1) {
        ...
        if (move0.isVestigial()) {
            move0._col1 = move1._col0;
            move0._row1 = move1._row0;
            move0._nextJump = move1;
            return move(move0._col0, move0._row0,
                    move1._col0, move1._row0, move1);
        }
        ...
        if (move0.jumpTail() == null) {
            ...
            Move temp = move0.jumpTail();
            while (temp.jumpTail() != null) {
                temp = temp.jumpTail();
            }
            temp._nextJump = move1;
            return move0;
        }
    ...
    /**
     * @param color is the current player
     * @return true iff this is a forward, non-capturing move.
     */
    boolean isForwardMove(PieceColor color) {
        if (color.equals(PieceColor.WHITE)) {
            return _col1 == _col0 && _row0 == _row1 - 1;
        } else {
            return _col1 == _col0 && _row0 == _row1 + 1;
        }
    }
    ...
    /**
     * @param color is current player.
     * @return true iff this is a diagonal, non-capturing move.
     */
    boolean isDiagMove(PieceColor color) {
        if (color.equals(PieceColor.WHITE)) {
            return (_col1 == _col0 + 1 || _col1 == _col0 - 1)
                    && _row0 == _row1 - 1;
        } else {
            return (_col1 == _col0 + 1 || _col1 == _col0 - 1)
                    && _row0 == _row1 + 1;
    ...
    char jumpedRow() {
        if (isJump()) {
            return (char) ((_row1 - _row0) / 2 + _row0);
        } else {
            return _row1;
        }
    }
    ...
    char jumpedCol() {
        if (isJump()) {
            return (char) ((_col1 - _col0) / 2 + _col0);
        } else {
            return _col1;
        }
    }
    ...
    private void toString(Formatter out) {
        out.format("%s%s-%s%s", col0(), row0(), col1(), row1());
        Move current = this.jumpTail();
        while (current != null) {
            out.format("-%s%s", current.col1(), current.row1());
            current = current.jumpTail();
        }
    }
