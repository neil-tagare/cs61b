...
class GUI extends TopLevel implements Observer, Reporter {
    ...
    GUI(String title, Board model, Writer outCommands) {
        ...
        layout = new LayoutSpec("height", "1",
                "width", "REMAINDER",
                "ileft", 2, "itop", 5, "iright", 8,
                "ibottom", 5);
        addLabel("test", "1", layout);
    ...
    /**
     * Execute the "restart" button function.
     */
    private synchronized void restart(String unused) {
        _out.printf("clear%n start%n");
    }
    ...
    /**
     * Execute the "rule" button function.
     */
     ...
     */
    private synchronized void rule(String unused) {
        displayText("qirkat/rules.txt", "help");
    }
    /**
     * Execute the "white ai" button function.
     */
    private synchronized void whiteai(String unused) {
        _out.printf("auto white%n");
    }

    /**
     * Execute the "white manual" button function.
     */
    private synchronized void whitemanual(String unused) {
        _out.printf("manual white%n");
    }

    /**
     * Execute the "black ai" button function.
     */
    private synchronized void blackai(String unused) {
        _out.printf("auto black%n");
    }

    /**
     * Execute the "black manual" button function.
     */
    private synchronized void blackmanual(String unused) {
    ...
    public void update(Observable obs, Object arg) {
        ...
        if (obs == _model) {
            ...
            if (!move.equals("")) {
                move = String.format("%s-%s", move, arg);
                selectMove(Move.parseMove(move));
            } else {
                move = (String) arg;
                selectMove(Move.parseMove(String.format("%s-%s", move, move)));
            }

            if (!_model.legalMove(_selectedMove)
                    && _model.checkJump(_selectedMove, false)) {
                return;
            } else if (!_model.legalMove(_selectedMove)
                    && !_selectedMove.isVestigial()) {
                move = "";
                selectMove(null);
            }
            movePiece(move);
    ...
    private void reportwinner() {
        if (_model.gameOver()) {
            showMessage(_model.whoseMove().opposite()
                    + " wins!", "Winner", "plain");
        }
    }
    ...
    private void whosemove() {
        setLabel("1", _model.whoseMove().toString() + " moves");
    }
