...
class CommandInterpreter {
    ...
    Table selectClause() {
        ...
        _input.next("from");
        Table fromtable = tableName();
        Table table;

        if (_input.nextIf("where")) {
            List<Condition> con = conditionClause(fromtable);
            table = fromtable.select(cns, con);
        } else if (_input.nextIf(",")) {
            Table fromtable2 = tableName();
            if (_input.nextIf("where")) {
                List<Condition> con = conditionClause(fromtable, fromtable2);
                table = fromtable.select(fromtable2, cns, con);
            } else {
                table = fromtable.select(fromtable2, cns, null);
            }
        } else {
            table = fromtable.select(cns, null);
        }
        return table;
    }
    ...
    Condition condition(Table... tables) {
        ...
        Column C1 = new Column(col1, tables);
        String relation = _input.next();
        if (_input.nextIs(LITERAL)) {
            String col2 = literal();
            return new Condition(C1, relation, col2);
        } else {
            String col2 = columnName();
            Column C2 = new Column(col2, tables);
