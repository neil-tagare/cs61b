        ...
        for (int i = 0; i < m.length; i++) {
            for (int j = 0; j < m[0].length; j++) {
                if (i == 0) {
                    ...
                    result[i][j] = Math.min(result[i-1][j], result[i-1][j+1])+m[i][j];
                } else if (j == m[0].length-1) {
                    result[i][j] = Math.min(result[i-1][j-1], result[i-1][j])+m[i][j];
                } else {
                    result[i][j] = Math.min(Math.min(result[i-1][j-1], result[i-1][j+1]), result[i-1][j]) + m[i][j];
                }
            }
        }
        return result;
